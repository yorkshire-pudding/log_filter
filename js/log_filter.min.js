...
_resize=function(evt,initially){var jq,o;
if(_.useModuleCss){o=(jq=$("#log_filter_criteria")).offset();
$("#page")[(o.left+jq.outerWidth(true)+$("div#log_filter_filters_cell_0").outerWidth(true))>(Judy.innerWidth(window)-20)?"addClass":"removeClass"]("log-filter-viewport-small");
}if(initially){Judy.overlay(0);
...
jq.change(function(){if(Judy.fieldValue(this)){_elements.conditions.uid.value="";
}_changedCriterion();
});
break;
...
$(_elements.filter.description.parentNode).hide();
}}if(_.crudFilters){$(_elements.filter.name_suggest.parentNode.parentNode).hide();
...
$(_elements.filter.name_suggest.parentNode.parentNode).show();
...
$(elm.parentNode.parentNode).show();
}else{$(_elements.filter.name_suggest.parentNode.parentNode).hide();
}$(_elements.filter.name_suggest.parentNode).hide();
$(_elements.filter.description.parentNode).show();
$(_elements.buttons.cancel).show();
$(_elements.buttons.save).show();
...
_filterByEventColumn=function(evt){var that=evt.target,tag,logId,col,log,u,o=_elements.conditions,elm,elm1,v,v1,a,vShow;
if(evt.type==="contextmenu"){evt.preventDefault();
}switch((tag=that.tagName||"none").toLowerCase()){case"td":break;
case"a":that=that.parentNode;
break;
default:return;
}if(!(col=that.getAttribute("log_filter_list_event_column"))){return;
}if(!(logId=that.parentNode.getAttribute("log_filter_list_event_id"))||!(log=_.logs["_"+logId])||!_.logs.hasOwnProperty("_"+logId)){return;
...
_ajaxRequest=function(action,oData){oData.form_token=Judy.fieldValue("[name='form_token']",_elements.form);
$.ajax({url:"/log_filter/ajax/"+action,type:"POST",data:oData,dataType:"json",cache:false,success:function(oResp,textStatus,jqXHR){var o;
...
return;
case _errorCodes.error_perm_filter_crud:self.Message.set(self.local("error_perm_filter_crud"),"warning",{modal:true,close:function(){window.location.href=_url();
}});
return;
...
if(!_stopped){if((_opacity=(n-=(_subtractor*=_factor)))>0){if(!_msie){jq.css("opacity",n/100);
...
if(_opacity<100){if(!_msie){__self._jq.css("opacity",1);
...
if((o=_.logs["_"+logId])&&_.logs.hasOwnProperty("_"+logId)){if((elm=document.getElementById(dialId))){$("#log_filter_list_log_"+logId).removeClass("log-filter-list-displayed");
...
if(typeof window.inspect==="function"&&window.inspect.formatInspectOutput){inspect.formatInspectOutput(true,dialInner);
inspect.formatTraceOutput(true,dialInner);
}$dialOuter.css({visibility:"visible",left:"150px",top:(4+_.adminOverlayOffset)+"px"});
}}};
this.init=function(useModuleCss,theme){self.init=function(){};
...
}_getLogList(/^.+\/(\d+)\/?$/.test(url=url)&&(wid=parseInt(url.replace(/^.+\/(\d+)\/?$/,"$1"),10))&&wid<=Math.pow(2,31)?wid:0);
